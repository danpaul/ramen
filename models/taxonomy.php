...
class Taxonomy_model extends Base_model
{
	...
	const STATEMENT_SELECT_CATEGORY = 'SELECT * FROM Categories WHERE name=:name LIMIT 1';
...
********************************************************************************/
	...
	*/
	protected function find_category_children($category_tree, $id)
	{
		...
		foreach ($category_tree as $category) {
			...
			if( $category['id'] === $id )
			{
				...
				{
					$children = $this->get_child_ids($category['subcategories'][0]);
				}
			}		
		}
		return $children;
	...
	{
		$params = array( 'name' => $name );
		$statement = $this->db->prepare(self::STATEMENT_SELECT_CATEGORY);
